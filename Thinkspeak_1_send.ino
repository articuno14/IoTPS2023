...
void loop() {
  ...
  if (newDataReady) {
    if (millis() > t + serialPrintInterval) {
      ...
      float w = 8.5; // Weight of one piece
      int k = int(i / w); // Calculate pieces based on weight

      Serial.print("Pieces Left: ");
      Serial.println(k);

      sendDataToThingSpeak(i, k);
...
void sendDataToThingSpeak(float weight, int pieces) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;

    String url = "http://" + String(server) + "/update?api_key=" + String(apiKey);
    url += "&field1=" + String(weight, 2); // 2 decimal places
